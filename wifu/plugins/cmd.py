...
from wifu.utils import download_file, unzip, save_config, parse_version, md5
from wifu.plugins import default_path


SAVE_DIR = '/usr/local/share/wifu/availaible-plugins/'
DEFAULT_INSTALL_PATH = '/usr/local/share/wifu/installed-plugins/'
...
def add_parsers(parser):
    ...
    ## wifu plugins search
    parser_plugins_search = plugin_subparsers.add_parser('search', help='Search for wifu plugins')
    parser_plugins_search.add_argument('pattern', type=str, help="Search expression (wildcards allowed)")

    ## wifu plugins list
    parser_plugins_list = plugin_subparsers.add_parser('list', help='List available wifu plugins')
...
def list_plugins(args, config, pattern='*'):
    ...
    if not found:
        logging.info('Maybe try: wifu plugins update')
        return 1
    return 0
