...
def toggle_plugin(name, enable=True):
    ...
    """
    import wifu
    from wifu.ui import view
    from wifu.utils import save_config

    global loaded, database

    if wifu.config:
        if not name in wifu.config['main']['plugins']:
            wifu.config['main']['plugins'][name] = dict()
        wifu.config['main']['plugins'][name]['enabled'] = enable
        save_config(wifu.config, '/etc/wifu/config.toml')
    ...
    if enable and name in database and name not in loaded:
        ...
        if name in loaded and wifu.config and name in wifu.config['main']['plugins']:
            loaded[name].options = wifu.config['main']['plugins'][name]
        one(name, 'loaded')
        if wifu.config:
            one(name, 'config_changed', wifu.config)
