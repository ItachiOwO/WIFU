...
import wifu.ui.hw as hw
from wifu.ui.view import View
...
class DisplayImpl(Display):
    def __init__(self, config, state={}, auto_flush=False):
        self.name = 'generic'
    ...
    def initialize(self):
        logging.info("initializing display %s (width: %d height: %d)" % (self.name, self._width, self._height))
