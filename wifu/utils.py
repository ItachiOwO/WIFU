...
def load_config(args):
    ...
    import wifu
    ref_defaults_file = os.path.join(os.path.dirname(wifu.__file__), 'defaults.toml')
    ...
    # check for an entire wifu folder on /boot/
    if os.path.isdir('/boot/wifu'):
        print("installing /boot/wifu to /etc/wifu ...")
        shutil.rmtree('/etc/wifu', ignore_errors=True)
        shutil.move('/boot/wifu', '/etc/')
...
def extract_from_pcap(path, fields):
    ...
    for field in fields:
        ...
        if field == WifiInfo.BSSID:
            ...
            from wifu.mesh.wifi import freq_to_channel
...
class StatusFile(object):
    ...
    def update(self, data=None):
        from wifu.fs import ensure_write
